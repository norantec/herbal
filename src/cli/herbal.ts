...
const getEntryFileContent: CreateForgeCommandOptions['getEntryFileContent'] = ({ entryFilePath }) => {
  return [
    ...
    "\n  if (workerData?.['__herbal_worker'] === true) {",
    '    entryOptions?.worker?.(resolver, finalListenPort);',
    '    return;',
    '  }',
    "\n  if (typeof entryOptions?.worker === 'function') new Worker(__filename, { workerData: { __herbal_worker: true } });",
