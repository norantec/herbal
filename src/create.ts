...
class TypeScriptClient extends Client implements Client {
  public constructor(public readonly options: CreateClientOptions) {
    super(options);
  }
  ...
  public generateClientSourceFile(currentGroup?: string) {
    ...
    return [
      ...
      '  url: K;',
      `  result: ${RESULT_TYPE_NAME}<${HTTP_RESPONSE_BODY_TYPE_NAME}<${METHOD_TYPE_MAP_NAME}[K]['response']>>;`,
      '};',
...
export function createTypeScriptClient(options: CreateClientOptions) {
